...
void trader_thread_function(uint32_t log_id)
{
    for (int i = 0; i < 10000; ++i)
    {
        ...
        logger.log(log_id, "Order %d price %.2f by Users %s, %s, %s, %s Order %d, %d", i, 123.45 + i, "Akshdeep", "Arshdeep", "Vaibhav", "Piyush", i, i * 2);
...
int main()
{
    ...
    int debugId = logger.create_log_file("debug", 31);
    ...
    //     t.join();
    trader_thread_function(debugId);
    logger.stop();
    // Signal consumer to exit as needed; omitted for brevity
    // logger.consumer.stop();
    // logger.consumer_thread.join();
}
